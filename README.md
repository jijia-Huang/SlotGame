...
The demo starts with a 6x5 cascading slot board, DOM HUD, local mock spin adapter, configurable anywhere-pay payouts, generated sequence symbols, and a Spine 3.8 loader entry point.
...
- `CascadeSpinAdapter` precomputes each cascade step so the animation never swaps random spin symbols into different stop symbols.
...
The current gameplay model follows a Storm-of-Seth-style tumble loop: fill a 6x5 board, pay 8+ matching symbols anywhere on the board, clear winners, drop/refill, and repeat until no win remains.
